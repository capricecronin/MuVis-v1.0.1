...
class AudioManager: ObservableObject {
    ...
    func processData(inputArray: [Float]) {
        ...
        var binBuffer = [Float](repeating: 0.0, count: AudioManager.binCount8)    // binCount8 = 3,022
        ...
        let binWidth: Float = ( Float(AudioManager.sampleRate) / 2.0 ) / Float(AudioManager.binCount)     //  (44,100/2) / 8,192 = 2.69165 Hz
        ...
        vDSP_vqint(binBuffer, &outputIndices, vDSP_Stride(1), &pointBuffer, vDSP_Stride(1), vDSP_Length(totalPointCount) , vDSP_Length(AudioManager.binCount8))
